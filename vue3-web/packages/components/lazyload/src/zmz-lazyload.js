...
Lazyload.install = function (Vue, options = {}) {
    ...
    var errorImage = options.errorImage || '';
    // 默认离可视区10px时加载图片
    ...
    const isCanShow = (item, binding) => {
        ...
        if ((typeof binding.value) == 'object' && binding.value.scrollTarget) {
            windowHeight = document.querySelector(binding.value.scrollTarget) && document.querySelector(binding.value.scrollTarget).offsetHeight;
        } else {
            if (scrollTarget) {
                windowHeight = document.querySelector(scrollTarget).offsetHeight;
            } else {
        ...
        if (binding.value.scrollTarget) {
            top = ele.getBoundingClientRect().top - document.querySelector(binding.value.scrollTarget).getBoundingClientRect().top;
        } else {
        ...
        if (parseInt(top) - distanece < windowHeight) {
            var image = new Image();
            image.src = src;
    ...
    const onListenScroll = (binding) => {
        if (binding.value.scrollTarget) {
            document.querySelector(binding.value.scrollTarget).onscroll = function () {
